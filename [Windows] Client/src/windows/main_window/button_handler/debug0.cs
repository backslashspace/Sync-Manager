...
internal static partial class MainWindow
{
    ...
    NEXT:
        ...
        if (node->Type == NodeType.File)
        {
            ...
            pathLengthBytes = GetPath(path, fileNode->ParentDirectoryBaseOffset, directoryBuffer, directoryStack);
        ...
        else if (node->Type == NodeType.HardLinkSlave)
        {
            HardLinkSlave* hardLinkSlave = (HardLinkSlave*)node;

            pathLengthBytes = GetPath(path, hardLinkSlave->ParentDirectoryBaseOffset, directoryBuffer, directoryStack);

            String parentPath = new(path, 0, pathLengthBytes >>> 1);
            String fileName = new(hardLinkSlave->Name, 0, hardLinkSlave->NameLengthBytes >>> 1);

            Log.Debug(parentPath + '\\' + fileName + "\n", Log.Level.Info, "HardLinkSlave");

            offset += hardLinkSlave->NextItemOffset;
        }
